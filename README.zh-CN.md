<div align="center">
  ...
  **在自己的电脑完成 AI 翻唱，并通过 Android 手机在局域网遥控。**
  ...
  ·
  [下载 Android APK](https://huggingface.co/hiruynk/RubaTone-AI-Local/resolve/main/RubaTone-AI-Local-1.0.2-arm64-release.apk)
  ·
...
1. 安装 [Android arm64 APK](https://huggingface.co/hiruynk/RubaTone-AI-Local/resolve/main/RubaTone-AI-Local-1.0.2-arm64-release.apk)。
2. 确保手机和电脑连接同一个私人网络。
3. 在电脑显示四位配对码。
4. 在手机选择电脑并输入配对码。
5. 在电脑确认手机名称和设备指纹。
...
- 更新只在 staging 重建变更文件；NTFS 使用硬链接沿用未变更 runtime，exFAT 则只以事务方式备份和替换变更文件，首次启动失败会自动回滚。
- 更新失败时保留并恢复上一个可启动版本。
...
| 组件 | 版本 | 状态 |
|---|---:|---|
| Windows Launcher | 1.0.9 | 已发布 |
| RTX 40 / 50 app / runtime | 1.0.4 | 已发布；RTX 5070 Ti 已验证 |
| Android arm64 APK | 1.0.2 | 已发布；等待实体手机矩阵验证 |

查看 [GitHub Release](https://github.com/Hiruynk/RubaTone-AI-Local/releases/tag/v1.0.9) 或 [Hugging Face 文件列表](https://huggingface.co/hiruynk/RubaTone-AI-Local/tree/main)。
